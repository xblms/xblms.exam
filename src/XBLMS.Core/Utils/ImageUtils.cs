...
namespace XBLMS.Core.Utils
{
    public static class ImageUtils
    {
        ...
        public static void AddTextWaterMark(string imagePath, string text, string fontName, int fontSize, int waterMarkPositionx, int waterMarkPositiony, int waterMarkOpacity, int minWidth, int minHeight)
        {
            try
            {
                using (var img = Image.Load(imagePath))
                {
                    ...
                    using (var img2 = img.Clone(ctx => ctx.ApplyTextWaterMark(font, text, waterMarkPositionx, waterMarkPositiony, waterMarkOpacity)))
                    {
                        img2.Save(imagePath);
                    }
                }
        ...
        public static void AddImageWaterMark(string imagePath, string waterMarkImagePath, int waterMarkPositionx, int waterMarkPositiony, int waterMarkTransparency, int minWidth, int minHeight)
        {
            try
            {
                using (var img = Image.Load(imagePath))
                {
                    ...
                    using (var img2 = img.Clone(ctx => ctx.ApplyImageWaterMark(waterMarkImagePath, waterMarkPositionx, waterMarkPositiony, waterMarkTransparency)))
                    {
                        img2.Save(imagePath);
                    }
                }
    ...
    Font font,
    string text,
    int waterMarkPositionx, int waterMarkPositiony,
    int waterMarkTransparency)
        {
            Size imgSize = processingContext.GetCurrentSize();
            var pointF = new PointF(waterMarkPositionx, waterMarkPositiony);
