using Aspose.Cells;
using System;
using System.IO;
using XBLMS.Utils;

namespace XBLMS.Core.Utils.Office
{
    public static class AsposeExcelObject
    {
        public static (string htmlUrl,bool success,string msg) GetPdfUrl(string fileUrl)
        {
            try
            {
                RemoveWatermark();
                var book = new Workbook(fileUrl);
                var excelFilname = PathUtils.GetFileName(fileUrl);
                var pdfFilename = PathUtils.GetFileNameWithoutExtension(fileUrl) + StringUtils.GetShortGuid() + ".pdf";

                var pdfPath = PathUtils.Combine(StringUtils.ReplaceEndsWithIgnoreCase(fileUrl, excelFilname, ""), pdfFilename);

                book.Save(pdfPath, SaveFormat.Pdf);
                return (pdfPath, true, "");
            }
            catch (Exception ex)
            {
                return ("", false, ex.Message);
            }
   
        }
        private static void RemoveWatermark()
        {
            new Aspose.Cells.License().SetLicense(new MemoryStream(Convert.FromBase64String("")));
        }
    }
}
