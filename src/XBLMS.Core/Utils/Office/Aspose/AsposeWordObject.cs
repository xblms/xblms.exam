using Aspose.Words;
using System;
using System.IO;
using XBLMS.Utils;

namespace XLMS.Core.Utils.Office
{
    public static class AsposeWordObject
    {
        private static void RemoveWatermark()
        {
            new Aspose.Words.License().SetLicense(new MemoryStream(Convert.FromBase64String("")));
        }

        public static (string url, bool success, string msg) GetPdfUrl(string fileUrl)
        {
            try
            {
                RemoveWatermark();
                Document doc = new Document(fileUrl);
                var docFilename = PathUtils.GetFileName(fileUrl);
                var pdfFilename = PathUtils.GetFileNameWithoutExtension(fileUrl) + StringUtils.GetShortGuid() + ".pdf";
                var pdfPath = PathUtils.Combine(StringUtils.ReplaceEndsWithIgnoreCase(fileUrl, docFilename, ""), pdfFilename);
                doc.Save(pdfPath, SaveFormat.Pdf);
                return (pdfPath, true, "");
            }
            catch (Exception ex)
            {
                return ("", false, ex.Message);
            }

        }
    }
}
